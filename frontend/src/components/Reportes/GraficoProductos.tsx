...
const GraficoProductos: React.FC<{ datos: Movimiento[] }> = ({ datos }) => {
  ...
  const productos = datos.reduce<Record<string, number[]>>((acc, { fecha, nombre, cantidad }) => {
  ...
  const colorMap = nombresProductos.reduce<Record<string, string>>((acc, nombre, i) => {
    acc[nombre] = colorPalette[i % colorPalette.length];
    return acc;
  }, {});
  ...
  return (
    ...
    >
      ...
      <div style={{ width: '80%', height: '80%' }}> {/* 60% más grande */}
...
>
  ...
  <div style={{ width: '60%', height: '60%' }}> {/* Ajustado al 60% */}
    <Doughnut
      data={doughnutData}
